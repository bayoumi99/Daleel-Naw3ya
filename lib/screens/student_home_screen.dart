...
class _StudentHomeScreenState extends State<StudentHomeScreen> {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            ...
            .where('target', whereIn: [userData?['year'] ?? '', 'جميع السنوات'])
            .orderBy('timestamp', descending: true)
            .snapshots(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
          var docs = snapshot.data?.docs ?? [];
          if (docs.isEmpty) return _buildEmptyStateCustom("لا توجد إشعارات حالياً");
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Text(userData?['name'] ?? "جاري التحميل..", style: const TextStyle(color: Colors.white, fontSize: 19, fontWeight: FontWeight.bold)),
                            Text("${userData?['year'] ?? ''} - ${userData?['department'] ?? ''}", style: const TextStyle(color: Colors.white60, fontSize: 12)),
                          ],
                        ),
                      ],
                    ),
  ...
  PersistentBottomNavBarItem _item(IconData icon, String title) {
    return PersistentBottomNavBarItem(
      icon: Icon(icon),
      title: title,
      activeColorPrimary: const Color(0xFF292F91),
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Icon(Icons.menu_book_rounded, color: widget.isDarkMode ? Colors.blueAccent : const Color(0xFF292F91)),
              Text("دليل نوعية", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: widget.isDarkMode ? Colors.white : const Color(0xFF292F91))),
            ],
          ),
        ),
      ),
  ...
  Widget _buildInfoCard(String title, String time, String room, String doctor) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(right: BorderSide(color: isDark ? Colors.blueAccent : const Color(0xFF292F91), width: 6)),
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(isDark ? 0.3 : 0.06), blurRadius: 15, offset: const Offset(0, 4))],
      ),
      child: Row(
        children: [
          Column(children: [Icon(Icons.access_time, size: 20, color: isDark ? Colors.blueAccent : const Color(0xFF292F91)), Text(time, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: isDark ? Colors.white70 : Colors.black87))]),
          const Spacer(),
          Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              Text(title, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: isDark ? Colors.blueAccent : const Color(0xFF292F91))),
              if(doctor.isNotEmpty) Text("دكتور: $doctor", style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 13)),
              Text("المكان: $room", style: TextStyle(color: isDark ? Colors.blue.shade200 : const Color(0xFF292F91), fontWeight: FontWeight.bold, fontSize: 13)),
            ],
          ),
        ],
      ),
  ...
  Widget _buildSectionHeader(String title) {
    return Padding(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Text(title, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: widget.isDarkMode? Colors.white : const Color(0xFF292F91))),
      ]),
    );
  }
}